        ...
        it(`Deve validar as metricas (mobile) para a URL: ${testUrl}`, () => {
           ...
           // const apiEndpoint = `https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url=${encodeURIComponent(testUrl)}&key=${apiKey}&strategy=mobile&category=${categories.join('&category=')}`;
            ...
            cy.api(apiEndpoint).then((response) => {
                ...
                expect(Trends.metricasDesempenho.desempenho).to.be.a('number').to.be.gte(0.1);
                expect(Trends.metricasDesempenho.acessibilidade).to.be.a('number').and.to.be.gte(0.1);
                expect(Trends.metricasDesempenho.melhoresPraticas).to.be.a('number').and.to.be.gte(0.1);
                ...
                expect(parseFloat(Trends.metricasCoreWeb.TTFB)).to.be.lte(9000); // TTFB Percentile deve ser menor ou igual a 800ms
                expect(parseFloat(Trends.metricasCoreWeb.FCP)).to.be.lte(18000); // FCP Percentile deve ser menor ou igual a 1.8s
                expect(parseFloat(Trends.metricasCoreWeb.INP)).to.be.lte(25000); // INP Percentile deve ser menor ou igual a 200ms
                expect(parseFloat(Trends.metricasCoreWeb.LCP)).to.be.lte(25000); // LCP Percentile deve ser menor ou igual a 2.5s
            });
        });
    });
});